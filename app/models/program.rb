class Program < ApplicationRecord
  ...
  scope :published,
        -> {
          where('application_start_time <= ?', Time.current).order(
            application_start_time: :desc,
          )
        }
  ...
  validates :application_start_time,
            date: {
              after_or_equal_to: Time.zone.local(2000, 1, 1),
              before: ->(obj) { 1.year.from_now },
              allow_blank: true,
            }
  validates :application_end_time,
            date: {
              after: :application_start_time,
              before: ->(obj) { obj.application_start_time.advance(days: 9) },
              allow_blank: true,
              if: ->(obj) { obj.application_start_time },
            }
  validates :min_number_of_participants,
  ...
  private def set_application_end_time
    if t = application_end_date&.in_time_zone
      self.application_end_time =
        t.advance(hours: application_end_hour, minutes: application_end_minute)
    end
  end
end
